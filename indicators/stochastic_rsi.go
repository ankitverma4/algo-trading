...
func NewFastStochasticIndicator(series *helpers.TimeSeries, timeframe int) Indicator {
	return kIndicator{
		closePrice: NewClosePriceIndicator(series),
		minValue:   NewRelativeStrengthIndicator(NewLowPriceIndicator(series), timeframe),
		maxValue:   NewRelativeStrengthIndicator(NewHighPriceIndicator(series), timeframe),
		window:     timeframe,
	}
}
